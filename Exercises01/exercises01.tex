...
\item % A
	...
	\begin{align}
		\frac{\nabla \ell (\beta)}{d\beta} &= \sum_{i=1}^N \left ( (m_i-y_i)x_i - m_i \frac{1}{1+\exp(-x_i^T\beta)}\exp({-x_i^T\beta})x_i \right ) \\
		&= \sum_{i=1}^N \left ( (m_i-y_i)x_i - m_i w_i\exp({-x_i^T\beta})x_i \right ) \\
		&= \sum_{i=1}^N  (m_i-y_i-m_i w_i\exp(-x_i^T\beta))x_i  \\
